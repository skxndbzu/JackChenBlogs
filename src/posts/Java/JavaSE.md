...
* 静态初始化:  `int[][] arr = {{11,22,33}, {44,55,66}}`
...
* 构造函数：以无参数构造方法创建 ArrayList 时，实际上初始化赋值的是一个空数组。当真正对数组进行添加元素操作时，才真正分配容量（惰性初始化），即向数组中添加第一个元素时，**数组容量扩为 10**，如果有参则按传参数分配初始容量
...
HashMap 基于哈希表的 Map 接口实现，默认容量是:16，是以 key-value 存储形式存在，主要用来存放键值对
