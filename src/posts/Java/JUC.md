...
![](https://jackchen-note.oss-cn-beijing.aliyuncs.com/Java/JUC/JUC-%E7%BA%BF%E7%A8%8B6%E7%A7%8D%E7%8A%B6%E6%80%81.png)
...
<img src="https://jackchen-note.oss-cn-beijing.aliyuncs.com/Java/JUC/JMM-%E5%86%85%E5%AD%98%E4%BA%A4%E4%BA%92.png" style="zoom: 67%;" />
...
* 读屏障（lfence，Load Barrier）保证在该屏障之后的，对共享变量的读取，从主存刷新变量值，加载的是主存中最新数据
  ...
  <img src="https://jackchen-note.oss-cn-beijing.aliyuncs.com/Java/JUC/JMM-volatile%E4%BF%9D%E8%AF%81%E5%8F%AF%E8%A7%81%E6%80%A7.png" style="zoom:67%;" />
...
![](https://jackchen-note.oss-cn-beijing.aliyuncs.com/Java/JUC/JMM-DCL%E5%87%BA%E7%8E%B0%E7%9A%84%E9%97%AE%E9%A2%98.png)
...
<img src="https://jackchen-note.oss-cn-beijing.aliyuncs.com/Java/JUC/image-20240423173222394.png" alt="image-20240423173222394" style="zoom:80%;" />
...
| public final int addAndGet(int data)  | 以原子方式将输入的数值与实例中的值相加并返回< br />实例：AtomicInteger 里的 value,返回新值 |
| public final int getAndAdd(int data)  | 以原子方式将输入的数值与实例中的值相加并返回< br />实例：AtomicInteger 里的 value,返回旧值 |
...
原子引用：对 Object(非基本数据类型的类) 进行原子操作，提供一种读和写都是原子性的对象引用变量

原子引用类：AtomicReference(给原子引用加上版本号)、AtomicStampedReference、AtomicMarkableReference(不关心是否更改过几次,只关心是否更改过)
...
![](https://jackchen-note.oss-cn-beijing.aliyuncs.com/Java/JUC/JUC-%E4%BC%AA%E5%85%B1%E4%BA%AB1.png)

@sun.misc.Contended：防止缓存行伪共享，在使用此注解的对象或字段的前后各增加 128 字节大小的 padding，使用 2 倍于大多数硬件缓存行让 CPU 将对象预读至缓存时**占用不同的缓存行**，这样就不会造成对方缓存行的失效

![](https://jackchen-note.oss-cn-beijing.aliyuncs.com/Java/JUC/JUC-%E4%BC%AA%E5%85%B1%E4%BA%AB2.png)
...
* LongAdder#add：累加方法
  ...
  ![image-20240424104632634](https://jackchen-note.oss-cn-beijing.aliyuncs.com/Java/JUC/image-20240424104632634.png)
...
* Striped64#longAccumulate：cell 数组创建
  ...
  ![image-20240424105506351](https://jackchen-note.oss-cn-beijing.aliyuncs.com/Java/JUC/image-20240424105506351.png)
  
  ![image-20240424105830074](https://jackchen-note.oss-cn-beijing.aliyuncs.com/Java/JUC/image-20240424105830074.png)
  
  ![image-20240424110148782](https://jackchen-note.oss-cn-beijing.aliyuncs.com/Java/JUC/image-20240424110148782.png)
...
![](https://jackchen-note.oss-cn-beijing.aliyuncs.com/Java/JUC/JUC-ThreadLocal%E6%95%B0%E6%8D%AE%E7%BB%93%E6%9E%84JDK8%E5%89%8D.png)
...
![](https://jackchen-note.oss-cn-beijing.aliyuncs.com/Java/JUC/JUC-ThreadLocal%E6%95%B0%E6%8D%AE%E7%BB%93%E6%9E%84JDK8%E5%90%8E.png)
...
* set()：添加数据，ThreadLocalMap 使用**线性探测法来解决哈希冲突**
  ...
  * 在探测过程中 ThreadLocal 会复用 key 为 null 的脏 Entry 对象，并进行垃圾清理，防止出现内存泄漏
  ...
  ![](https://jackchen-note.oss-cn-beijing.aliyuncs.com/Java/JUC/JUC-replaceStaleEntry%E6%B5%81%E7%A8%8B.png)
...
* 探测式清理：沿着开始位置向后探测清理过期数据，沿途中碰到未过期数据则将此数据 rehash 在 table 数组中的定位，重定位后的元素理论上更接近 `i = entry.key & (table.length - 1)`，让**数据的排列更紧凑**，会优化整个散列表查询性能
  ...
  <img src="https://jackchen-note.oss-cn-beijing.aliyuncs.com/Java/JUC/JUC-ThreadLocal%E6%8E%A2%E6%B5%8B%E5%BC%8F%E6%B8%85%E7%90%861.png" style="zoom:67%;" />

  <img src="https://jackchen-note.oss-cn-beijing.aliyuncs.com/Java/JUC/JUC-ThreadLocal%E6%8E%A2%E6%B5%8B%E5%BC%8F%E6%B8%85%E7%90%862.png" style="zoom:67%;" />
...
* 如果 key 使用强引用：使用完 ThreadLocal ，threadLocal Ref 被回收，但是 threadLocalMap 的 Entry 强引用了 threadLocal，造成 threadLocal 无法被回收，无法完全避免内存泄漏
  ...
  <img src="https://jackchen-note.oss-cn-beijing.aliyuncs.com/Java/JUC/JUC-ThreadLocal%E5%86%85%E5%AD%98%E6%B3%84%E6%BC%8F%E5%BC%BA%E5%BC%95%E7%94%A8.png" style="zoom:67%;" />
...
* 如果 key 使用弱引用：使用完 ThreadLocal ，threadLocal Ref 被回收，ThreadLocalMap 只持有 ThreadLocal 的弱引用，所以threadlocal 也可以被回收，此时 Entry 中的 key = null。但没有手动删除这个 Entry 或者 CurrentThread 依然运行，依然存在强引用链，value 不会被回收，而这块 value 永远不会被访问到，也会导致 value 内存泄漏
  ...
  <img src="https://jackchen-note.oss-cn-beijing.aliyuncs.com/Java/JUC/JUC-ThreadLocal%E5%86%85%E5%AD%98%E6%B3%84%E6%BC%8F%E5%BC%B1%E5%BC%95%E7%94%A8.png" style="zoom:67%;" />
...
* 初始化链表 `last = head = new Node< E>(null)`，**Dummy 节点(哑元节点)用来占位**，item 为 null
...
* 当一个节点入队：
  ...
  ![](https://jackchen-note.oss-cn-beijing.aliyuncs.com/Java/JUC/JUC-LinkedBlockingQueue%E5%85%A5%E9%98%9F%E6%B5%81%E7%A8%8B.png)
...
* `h = head` → `first = h.next` 
  ...
  ![](https://jackchen-note.oss-cn-beijing.aliyuncs.com/Java/JUC/JUC-LinkedBlockingQueue%E5%87%BA%E9%98%9F%E6%B5%81%E7%A8%8B1.png)

* `h.next = h` → `head = first`

  ![](https://jackchen-note.oss-cn-beijing.aliyuncs.com/Java/JUC/JUC-LinkedBlockingQueue%E5%87%BA%E9%98%9F%E6%B5%81%E7%A8%8B2.png)

  * E x =  first.item : 把数据取出来
...
![](https://jackchen-note.oss-cn-beijing.aliyuncs.com/Java/JUC/JUC-%E7%BA%BF%E7%A8%8B%E6%B1%A0%E5%B7%A5%E4%BD%9C%E5%8E%9F%E7%90%86.png)
...
![](https://jackchen-note.oss-cn-beijing.aliyuncs.com/Java/JUC/JUC-newSingleThreadExecutor.png)
...
* 状态表示：
  ...
  ![](https://jackchen-note.oss-cn-beijing.aliyuncs.com/Java/JUC/JUC-%E7%BA%BF%E7%A8%8B%E6%B1%A0%E7%8A%B6%E6%80%81%E8%BD%AC%E6%8D%A2%E5%9B%BE.png)
...
* 线程池相关参数：
  ...
  private volatile TimeUnit unit;			        // 时间单位
...
* 请求的共享资源被占用，AQS 用队列实现线程阻塞等待以及被唤醒时锁分配的机制，将暂时获取不到锁的线程加入到队列中
  ...
  <img src="https://jackchen-note.oss-cn-beijing.aliyuncs.com/Java/JUC/JUC-AQS%E5%8E%9F%E7%90%86%E5%9B%BE.png" style="zoom: 80%;" />
...
* 使用了 FIFO 先入先出队列，并不支持优先级队列，**同步队列是双向链表，便于出队入队**
  ...
  ![](https://jackchen-note.oss-cn-beijing.aliyuncs.com/Java/JUC/JUC-AQS%E9%98%9F%E5%88%97%E8%AE%BE%E8%AE%A1.png)
...
<img src="https://jackchen-note.oss-cn-beijing.aliyuncs.com/Java/JUC/JUC-ReentrantLock-%E9%9D%9E%E5%85%AC%E5%B9%B3%E9%94%811.png" style="zoom:80%;" />
...
* 接下来进入 addWaiter 逻辑，构造 Node 队列（不是阻塞队列），前置条件是当前线程获取锁失败，说明有线程占用了锁
  ...
  * Node 的创建是懒惰的，其中第一个 Node 称为 **Dummy（哑元）或哨兵**，用来占位，并不关联线程
  ...
  <img src="https://jackchen-note.oss-cn-beijing.aliyuncs.com/Java/JUC/JUC-ReentrantLock-%E9%9D%9E%E5%85%AC%E5%B9%B3%E9%94%812.png" style="zoom:80%;" />
...
* 再有多个线程经历竞争失败后：
  ...
  ![](https://jackchen-note.oss-cn-beijing.aliyuncs.com/Java/JUC/JUC-ReentrantLock-%E9%9D%9E%E5%85%AC%E5%B9%B3%E9%94%813.png)
...
* 唤醒的线程会从 park 位置开始执行，如果加锁成功（没有竞争），会设置
  ...
  * 原本的 head 因为从链表断开，而可被垃圾回收（图中有错误，原来的头节点的 waitStatus 被改为 0 了）
  ...
  ![](https://jackchen-note.oss-cn-beijing.aliyuncs.com/Java/JUC/JUC-ReentrantLock-%E9%9D%9E%E5%85%AC%E5%B9%B3%E9%94%814.png)
...
* 如果这时有其它线程来竞争**（非公平）**，例如这时有 Thread-4 来了并抢占了锁
  ...
  * Thread-1 再次进入 acquireQueued 流程，获取锁失败，重新进入 park 阻塞
  ...
  ![](https://jackchen-note.oss-cn-beijing.aliyuncs.com/Java/JUC/JUC-ReentrantLock-%E9%9D%9E%E5%85%AC%E5%B9%B3%E9%94%815.png)
...
* 开始 Thread-0 持有锁，调用 await，线程进入 ConditionObject 等待，直到被唤醒或打断，调用 await 方法的线程都是持锁状态的，所以说逻辑里**不存在并发**
  ...
  ![](https://jackchen-note.oss-cn-beijing.aliyuncs.com/Java/JUC/JUC-ReentrantLock-%E6%9D%A1%E4%BB%B6%E5%8F%98%E9%87%8F1.png)
...
* fullyRelease 中会 unpark AQS 队列中的下一个节点竞争锁，假设 Thread-1 竞争成功
  ...
  ![](https://jackchen-note.oss-cn-beijing.aliyuncs.com/Java/JUC/JUC-ReentrantLock-%E6%9D%A1%E4%BB%B6%E5%8F%98%E9%87%8F2.png)
...
* 执行 transferForSignal，**先将节点的 waitStatus 改为 0，然后加入 AQS 阻塞队列尾部**，将 Thread-3 的 waitStatus 改为 -1
  ...
  final boolean transferForSignal(Node node) {
      ...
      // 如果前驱节点被取消(ws > 0)或者不能设置状态为 Node.SIGNAL，就 unpark 取消当前节点线程的阻塞状态, 
  ...
  ![](https://jackchen-note.oss-cn-beijing.aliyuncs.com/Java/JUC/JUC-ReentrantLock-%E6%9D%A1%E4%BB%B6%E5%8F%98%E9%87%8F3.png)
...
* 补充情况：查询线程 A 查询数据时恰好缓存数据由于时间到期失效，或是第一次查询
  ...
  <img src="https://jackchen-note.oss-cn-beijing.aliyuncs.com/Java/JUC/JUC-ReentrantReadWriteLock%E7%BC%93%E5%AD%98.png" style="zoom:80%;" />
...
* 获取读锁失败，进入 sync.doAcquireShared(1) 流程开始阻塞，首先也是调用 addWaiter 添加节点，不同之处在于节点被设置为 Node.SHARED 模式而非 Node.EXCLUSIVE 模式，注意此时 t2 仍处于活跃状态
  ...
  <img src="https://jackchen-note.oss-cn-beijing.aliyuncs.com/Java/JUC/JUC-ReentrantReadWriteLock%E5%8A%A0%E9%94%811.png" style="zoom: 80%;" />

* 这种状态下，假设又有 t3 r.lock，t4 w.lock，这期间 t1 仍然持有锁，就变成了下面的样子

  ![](https://jackchen-note.oss-cn-beijing.aliyuncs.com/Java/JUC/JUC-ReentrantReadWriteLock%E5%8A%A0%E9%94%812.png)
...
* 接下来 t2 调用 setHeadAndPropagate(node, 1)，它原本所在节点被置为头节点；还会检查下一个节点是否是 shared，如果是则调用 doReleaseShared() 将 head 的状态从 -1 改为 0 并唤醒下一个节点，这时 t3 在 doAcquireShared 内 parkAndCheckInterrupt() 处恢复运行，**唤醒连续的所有的共享节点**
  ...
  <img src="https://jackchen-note.oss-cn-beijing.aliyuncs.com/Java/JUC/JUC-ReentrantReadWriteLock%E8%A7%A3%E9%94%811.png" style="zoom: 67%;" />
...
* t4 在 acquireQueued 中 parkAndCheckInterrupt 处恢复运行，再次 for (;;) 这次自己是头节点的临节点，并且没有其他节点竞争，tryAcquire(1) 成功，修改头结点，流程结束
  ...
  <img src="https://jackchen-note.oss-cn-beijing.aliyuncs.com/Java/JUC/JUC-ReentrantReadWriteLock%E8%A7%A3%E9%94%812.png" style="zoom: 67%;" />
  
  ![image-20240428131618121](https://jackchen-note.oss-cn-beijing.aliyuncs.com/Java/JUC/image-20240428131618121.png)
...
<img src="https://jackchen-note.oss-cn-beijing.aliyuncs.com/Java/JUC/JUC-CyclicBarrier%E5%B7%A5%E4%BD%9C%E5%8E%9F%E7%90%86.png" style="zoom: 80%;" />
...
* Semaphore 的 permits（state）为 3，这时 5 个线程来获取资源
  ...
  ![](https://jackchen-note.oss-cn-beijing.aliyuncs.com/Java/JUC/JUC-Semaphore%E5%B7%A5%E4%BD%9C%E6%B5%81%E7%A8%8B1.png)
...
* 这时 Thread-4 释放了 permits，状态如下
  ...
  ![](https://jackchen-note.oss-cn-beijing.aliyuncs.com/Java/JUC/JUC-Semaphore%E5%B7%A5%E4%BD%9C%E6%B5%81%E7%A8%8B2.png)
...
![ConcurrentHashMap数据结构](https://jackchen-note.oss-cn-beijing.aliyuncs.com/Java/JUC/ConcurrentHashMap%E6%95%B0%E6%8D%AE%E7%BB%93%E6%9E%84.png)
...
* 多个参数构造方法：
  ...
  public ConcurrentHashMap(int initialCapacity, float loadFactor, int concurrencyLevel) {
      ...
      // tableSizeFor仍然保证计算的大小是2的n次方,即16,32,64
...
* putVal()
  ...
  final V putVal(K key, V value, boolean onlyIfAbsent) {
      ...
      for (Node< K,V>[] tab = table;;) {
          ...
          if (tab == null || (n = tab.length) == 0)
              //【延迟初始化】使用了cas,无需synchronized创建成功,进入下一轮循环
              tab = initTable();
          
          // 【CASE2】：i 表示 key 使用【寻址算法】得到 key 对应数组的下标位置，tabAt 获取指定桶位的头结点f
          else if ((f = tabAt(tab, i = (n - 1) & hash)) == null) {
              // 对应的数组为 null 说明没有哈希冲突，直接新建节点添加到表中,使用了cas,无需synchronized
...
* transfer()：数据转移到新表中，完成扩容
  ...
  ![](https://jackchen-note.oss-cn-beijing.aliyuncs.com/Java/JUC/JUC-ConcurrentHashMap-LastRun%E6%9C%BA%E5%88%B6.png)
...
* 缺点：Segments 数组默认大小为16，这个容量初始化指定后就不能改变了，并且不是懒惰初始化
  ...
  ![](https://jackchen-note.oss-cn-beijing.aliyuncs.com/Java/JUC/JUC-ConcurrentHashMap%201.7%E5%BA%95%E5%B1%82%E7%BB%93%E6%9E%84.png)
...
<img src="https://jackchen-note.oss-cn-beijing.aliyuncs.com/Java/JUC/JUC-CopyOnWriteArrayList%E5%BC%B1%E4%B8%80%E8%87%B4%E6%80%A7.png" style="zoom:80%;" />
...
![](https://jackchen-note.oss-cn-beijing.aliyuncs.com/Java/JUC/JUC-ConcurrentSkipListMap%E6%95%B0%E6%8D%AE%E7%BB%93%E6%9E%84.png)
...
* findPredecessor()：寻找前置节点
  ...
  ![](https://jackchen-note.oss-cn-beijing.aliyuncs.com/Java/JUC/JUC-ConcurrentSkipListMap-Put%E6%B5%81%E7%A8%8B.png)
...
* remove()
  ...
  ![](https://jackchen-note.oss-cn-beijing.aliyuncs.com/Java/JUC/JUC-ConcurrentSkipListMap-remove%E6%B5%81%E7%A8%8B.png)
...
![](https://jackchen-note.oss-cn-beijing.aliyuncs.com/Java/JUC/JUC-ConcurrentLinkedQueue%E5%85%A5%E9%98%9F%E6%93%8D%E4%BD%9C1.png)

![](https://jackchen-note.oss-cn-beijing.aliyuncs.com/Java/JUC/JUC-ConcurrentLinkedQueue%E5%85%A5%E9%98%9F%E6%93%8D%E4%BD%9C2.png)

![](https://jackchen-note.oss-cn-beijing.aliyuncs.com/Java/JUC/JUC-ConcurrentLinkedQueue%E5%85%A5%E9%98%9F%E6%93%8D%E4%BD%9C3.png)
...
![](https://jackchen-note.oss-cn-beijing.aliyuncs.com/Java/JUC/JUC-ConcurrentLinkedQueue%E5%87%BA%E9%98%9F%E6%93%8D%E4%BD%9C1.png)

![](https://jackchen-note.oss-cn-beijing.aliyuncs.com/Java/JUC/JUC-ConcurrentLinkedQueue%E5%87%BA%E9%98%9F%E6%93%8D%E4%BD%9C2.png)

![](https://jackchen-note.oss-cn-beijing.aliyuncs.com/Java/JUC/JUC-ConcurrentLinkedQueue%E5%87%BA%E9%98%9F%E6%93%8D%E4%BD%9C3.png)
...
![](https://jackchen-note.oss-cn-beijing.aliyuncs.com/Java/JUC/IO%E6%A8%A1%E5%9E%8B-%E9%98%BB%E5%A1%9E%E5%BC%8FIO.png)
...
![](https://jackchen-note.oss-cn-beijing.aliyuncs.com/Java/JUC/IO%E6%A8%A1%E5%9E%8B-%E9%9D%9E%E9%98%BB%E5%A1%9E%E5%BC%8FIO.png)
...
![](https://jackchen-note.oss-cn-beijing.aliyuncs.com/Java/JUC/IO%E6%A8%A1%E5%9E%8B-%E4%BF%A1%E5%8F%B7%E9%A9%B1%E5%8A%A8IO.png)
...
![](https://jackchen-note.oss-cn-beijing.aliyuncs.com/Java/JUC/IO%E6%A8%A1%E5%9E%8B-IO%E5%A4%8D%E7%94%A8%E6%A8%A1%E5%9E%8B.png)
...
![](https://jackchen-note.oss-cn-beijing.aliyuncs.com/Java/JUC/IO%E6%A8%A1%E5%9E%8B-%E5%BC%82%E6%AD%A5IO%E6%A8%A1%E5%9E%8B.png)
...
![](https://jackchen-note.oss-cn-beijing.aliyuncs.com/Java/JUC/IO-select%E8%B0%83%E7%94%A8%E8%BF%87%E7%A8%8B.png)
...
![](https://jackchen-note.oss-cn-beijing.aliyuncs.com/Java/JUC/IO-%E7%94%A8%E6%88%B7%E6%80%81%E5%92%8C%E5%86%85%E6%A0%B8%E6%80%81.png)
...
![](https://jackchen-note.oss-cn-beijing.aliyuncs.com/Java/JUC/IO-%E7%B3%BB%E7%BB%9F%E8%B0%83%E7%94%A8%E7%9A%84%E8%BF%87%E7%A8%8B.jpg)
...
<img src="https://jackchen-note.oss-cn-beijing.aliyuncs.com/Java/JUC/IO-DMA.png" style="zoom: 50%;" />
...
![](https://jackchen-note.oss-cn-beijing.aliyuncs.com/Java/JUC/IO-BIO%E5%B7%A5%E4%BD%9C%E6%B5%81%E7%A8%8B.png)

read 调用图示：read、write 都是系统调用指令

<img src="https://jackchen-note.oss-cn-beijing.aliyuncs.com/Java/JUC/IO-%E7%BC%93%E5%86%B2%E5%8C%BA%E8%AF%BB%E5%86%99.png" style="zoom: 67%;" />
...
![](https://jackchen-note.oss-cn-beijing.aliyuncs.com/Java/JUC/IO-mmap%E5%B7%A5%E4%BD%9C%E6%B5%81%E7%A8%8B.png)
...
![](https://jackchen-note.oss-cn-beijing.aliyuncs.com/Java/JUC/IO-sendfile%E5%B7%A5%E4%BD%9C%E6%B5%81%E7%A8%8B.png)
...
<img src="https://jackchen-note.oss-cn-beijing.aliyuncs.com/Java/JUC/%E4%B8%89%E6%AC%A1%E6%8F%A1%E6%89%8B.png" alt="三次握手" style="zoom: 50%;" />

<img src="https://jackchen-note.oss-cn-beijing.aliyuncs.com/Java/JUC/%E5%9B%9B%E6%AC%A1%E6%8C%A5%E6%89%8B.png" alt="四次挥手" style="zoom: 67%;" />
...
* 常用 API：`public Socket accept()`，**阻塞等待**接收一个客户端的 Socket 管道连接请求，连接成功返回一个 Socket 对象
  ...
  <img src="https://jackchen-note.oss-cn-beijing.aliyuncs.com/Java/JUC/Netty-TCP%E4%B8%89%E6%AC%A1%E6%8F%A1%E6%89%8B.png" style="zoom:67%;" />
...
![](https://jackchen-note.oss-cn-beijing.aliyuncs.com/Java/JUC/BIO%E5%B7%A5%E4%BD%9C%E6%9C%BA%E5%88%B6.png)

![](https://jackchen-note.oss-cn-beijing.aliyuncs.com/Java/JUC/TCP-%E5%B7%A5%E4%BD%9C%E6%A8%A1%E5%9E%8B.png)
...
![](https://jackchen-note.oss-cn-beijing.aliyuncs.com/Java/JUC/NIO%E6%A1%86%E6%9E%B6.png)
...
![](https://jackchen-note.oss-cn-beijing.aliyuncs.com/Java/JUC/NIO-Buffer.png)
...
* 位置、限制、容量遵守以下不变式： **0 < = position < = limit < = capacity**
  ...
  <img src="https://jackchen-note.oss-cn-beijing.aliyuncs.com/Java/JUC/NIO-Buffer%E6%93%8D%E4%BD%9C.png" style="zoom:67%;" />
...
<img src="https://jackchen-note.oss-cn-beijing.aliyuncs.com/Java/JUC/JVM-%E7%9B%B4%E6%8E%A5%E5%86%85%E5%AD%98%E7%9B%B4%E6%8E%A5%E7%BC%93%E5%86%B2%E5%8C%BA.png" style="zoom: 50%;" />

<img src="https://jackchen-note.oss-cn-beijing.aliyuncs.com/Java/JUC/JVM-%E7%9B%B4%E6%8E%A5%E5%86%85%E5%AD%98%E9%9D%9E%E7%9B%B4%E6%8E%A5%E7%BC%93%E5%86%B2%E5%8C%BA.png" style="zoom:50%;" />
...
![](https://jackchen-note.oss-cn-beijing.aliyuncs.com/Java/JUC/NIO-%E5%A4%8D%E5%88%B6%E6%96%87%E4%BB%B6.png)
...
![](https://jackchen-note.oss-cn-beijing.aliyuncs.com/Java/JUC/NIO-Selector.png)
